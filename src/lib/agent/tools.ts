...
const markSpecSchema = z.object({
  type: z.string().describe("The mark type: barY, barX, dot, line, lineY, areaY, cell, rect, rectX, rectY, text, tickX, tickY, ruleX, ruleY, frame, tip, axisX, axisY, axisFx, axisFy"),
  data: z.string().optional().describe('Use "csv" to reference the uploaded dataset'),
  options: z.record(z.string(), z.unknown()).optional().describe("Mark options: x, y, fill, stroke, tip, transforms, etc."),
});
